...
见 `examples/1.2.3_TCP-IP模型_智能版.docx`。对数量不做硬性要求，只要内容足够——每个知识点都有截图、表格、公式、做题要点、WHY 解释即可。
...
13. **"长得像但内容不同"的帧会丢**：同一张PPT上不同定义（如"实体+协议" vs "接口+服务"），单轮vision按视觉相似分组会只留一张。解决：用两轮vision，第一轮按知识点分组，第二轮每组选最完整版。
